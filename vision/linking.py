    ...
    if waiting:
        for track, bus in zip(waiting, _pending_entries(linked, window)):
            ...
            ParkingEvent.objects.create(
                ...
                message=f"Camera {track.camera_id} matched track #{track.track_id} to {bus.bus_number}",
            )
...
def _apply_occupancy(tracks, sensor):
    ...
    desired = {t.slot_id: t.bus_id for t in tracks if t.bus_id and t.slot_id}
    if not desired:
        return 0

    bus_ids = set(desired.values())
    previous = dict(
    ...
    for slot_id, bus_id in desired.items():
        ParkingSlot.objects.filter(pk=slot_id).update(bus_id=bus_id, is_occupied=True)
    ...
    for slot_id, bus_id in desired.items():
        if previous.get(bus_id) != slot_id:
            ...
            ParkingEvent.objects.create(
                ...
                message=f"Camera placed {buses[bus_id].bus_number} at {slot.row}{slot.slot_number}",
            )
    return moved
...
def process_frame(camera_id, session, detections):
    ...
    VisionTrack.objects.filter(
        camera_id=camera_id, is_active=True, last_seen__lt=now - timedelta(seconds=timeout)
    ).update(is_active=False)
    VisionTrack.objects.filter(is_active=False, last_seen__lt=now - timedelta(days=1)).delete()
